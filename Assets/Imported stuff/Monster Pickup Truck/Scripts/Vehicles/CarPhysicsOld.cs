...
public class CarPhysicsOld : MonoBehaviour {
	...
	void CalculateSteering()
	{
		...
		float frontMaxSteering = Mathf.Clamp(maxSteering, tPeakAngle / Mathf.Lerp(1, frontWheels, cSteerWeight), cSteerMax);
		float rearMaxSteering = Mathf.Clamp(maxSteering, tPeakAngle / Mathf.Lerp(1, rearWheels, cSteerWeight), cSteerMax);
		...
		float finalSteeringFront = Mathf.Clamp(assistSteeringFront * (/*(1 - Mathf.Abs(steerInput)) */ (1 - cSteerWeight)) + ((frontMaxSteering) * steerInput), -frontMaxSteering, frontMaxSteering);
		...
		float rearSteeringAngle = ((rearMaxSteering) * steerInput);
	...
	void RunSuspension(int i, float gripBias)
	{
		...
		wheels[i].suspensionMult = springLerp02;
